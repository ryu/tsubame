...
1. **バリデーション時** — IP リテラル URL (http://127.0.0.1 等) をモデルバリデーションで拒否。IPv4/IPv6 両対応
2. **フェッチ時** — ホスト名を DNS 解決し、プライベート IP (10.x, 172.16-31.x, 192.168.x, 127.x, 169.254.x, ::1, fc00::/7) を拒否。リダイレクト先も同様にチェック

- レスポンスサイズ上限: 10MB

## クロール間隔

- フィードごとに設定可能（デフォルト: 6時間）
