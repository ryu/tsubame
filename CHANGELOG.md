...
### Fixed
- 既に登録済みのフィードを再度登録しようとした場合に「既に登録済みのフィードです。」と表示するよう修正
...
### Fixed
- サイドバー未読バッジの陳腐化を修正: cross-feed 既読同期後、エントリーリストが空なのにバッジが古い値を示す問題を解消
- 個別エントリー既読時にフォルダ未読バッジが更新されない問題を修正（差し引きでなく配下フィード合算で再計算）

### Changed
- 直接クリックでフィードを開いた場合もキーボードナビ用インデックスを正しく同期
